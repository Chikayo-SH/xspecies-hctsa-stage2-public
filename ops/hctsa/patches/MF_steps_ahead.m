...
% --- Ensure System Identification Toolbox predict is used (avoid shadowing)
identDir = fullfile(matlabroot, "toolbox", "ident", "ident");
addpath(identDir, "-begin");


for i = 1:maxSteps
    % (1) *** Model m ***

    % Ensure iddata input for ident predict
    z = iddata(yTest.y, [], 1);  % Ts=1 (fs=1前提)
    ...
    % (2) *** Sliding mean 1 *** A sliding mean of length 1 for n-step-ahead
    % prediction, will predict using the value n steps before it.
