...
% ------------------------------------------------------------------------------
% --- Guard against impossible/unstable embedding (prevents false_nearest segfault) ---
N = length(y);
Neff = N - (maxm-1)*tau;

% need at least 1 point to embed, but in practice false_nearest becomes unstable if Neff is too small
if Neff <= 0
    error('TISEAN false_nearest: N too short (N=%d) for tau=%d and maxm=%d', N, tau, maxm);
end

% stability margin (empirical): require a minimum number of embedded vectors
minNeff = 50;
if Neff < minNeff
    error('TISEAN false_nearest: Neff too small (Neff=%d, N=%d) for tau=%d and maxm=%d', Neff, N, tau, maxm);
end
