...
for ich = 1:numel(tgtChannels)
    ...
    tic;
    fprintf("TS_Compute START: %s\n", hctsaName);
    compute_stdout = "";
    try
        compute_stdout = evalc('TS_Compute(true, [], [], [], hctsaName);');
    catch ME
        dbg_log = strrep(hctsaName, '.mat', '_TS_Compute_stdout.log');
        fid = fopen(dbg_log, 'w');
        if fid ~= -1
            fprintf(fid, '%s', compute_stdout);
            fclose(fid);
        end
        rethrow(ME);
    end
